...
AS $$   
    ...
    INSERT INTO Courses (course_id,course_area_name,title,course_description,duration) VALUES (id,course_area_name,title,course_description,duration)
$$ LANGUAGE SQL
...
(IN session_date DATE,start_time TIME,duration INTEGER)
RETURN TABLE(rid INTEGER) AS $$
    SELECT rid 
    FROM Rooms 
    ...
    OR 
    -- and rooms occupied where duration overlaps
    (extract(hour from start_time) + duration > C.start_time)
    )
